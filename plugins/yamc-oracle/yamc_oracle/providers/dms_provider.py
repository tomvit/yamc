...
class DmsProvider(BaseProvider):
    ...
    def init_dms(self):
        if self.dms is None or time.time()-self.connect_time > self.reconnect_after:
            ...
            self.log.info("DMS provider initialized: url=%s, username=%s, password=(secret)"%(self.admin_url,self.username))
    ...
    def table(self,table,include=[],exclude=[],filter=None):
        ...
        self.log.info(f"The DMS table operation retrieved {len(data)} records in {d['query_time']:0.4f} seconds from '{table}'.")
