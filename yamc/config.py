...
class Config():
    ...
    def __init__(self, file, args):
        ...
        self.raw_config, self.config_file, self.config_dir = read_raw_config(self.args.config, self.args.env)
        self.logs_dir, self.logs_level = self.init_logging(deep_find(self.raw_config, "directories.logs",default="../logs"))
    ...
    def collector(self,collector_id):
        ...
        if collector_id not in self.collectors:
            self.collectors[collector_id] = ConfigPart(self, "collectors.%s"%collector_id, self.config._config, self.config_dir)
    ...
    def provider(self,provider_id):
        ...
        if provider_id not in self.providers:
            self.providers[provider_id] = ConfigPart(self, "providers.%s"%provider_id, self.config._config, self.config_dir)
            ...
            if val == default:
                ...
                if not no_eval:
                    if callable(getattr(val, "eval", None)):
                        try:
                            from yamc import yamc_scope
                            val = val.eval(merge_dicts(self.parent.custom_functions, yamc_scope))
                        except Exception as e:
                            raise Exception("Cannot evaluate Python expression for property '%s'. %s"%(self.path(path), str(e)))
