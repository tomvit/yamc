        ...
        if not self.args.test:
            ...
            try:
                while not exit_event.is_set():
                    if GPIO.wait_for_edge(int(self.gpio_channel), GPIO.RISING, timeout=self.timeout_rising_edge):
                        if GPIO.wait_for_edge(int(self.gpio_channel), GPIO.FALLING, timeout=self.timeout_falling_edge):
                            self._pulses += 1
                        else:
