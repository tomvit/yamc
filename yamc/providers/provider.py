...
class HttpProvider(BaseProvider):
    ...
    def init_session(self):
        try:
            if self.init_url is not None and (self.init_time is None or time.time()-self.init_time>self.init_max_age):
            ...
            if self.updated_time is None or self.data is None or time.time()-self.updated_time>self.max_age:
                ...
                while num_retries<3:
                    ...
                    if r.status_code == 404:
                        ...
                        self.log.error("The request at %s failed, status-code=%d, num-retries=%d"%(self.url, r.status_code,num_retries))
                        num_retries += 1
                        if num_retries == 3:
                            raise Exception("Cannot retrieve the resource at %s after %d attempts!"%(self.url,num_retries))
                        time.sleep(1)
                    else:
                        break
    ...
    def xpath(self, xpath, diff=False):
        ...
        if isinstance(d,list):
            if len(d)>0:
                ...
                self.log.error("The xpath '%s' cannot be evaluated on the following data: %s"%(xpath,str(self.content.decode(self.encoding))))
