...
class Writer(WorkerComponent):
    ...
    def worker(self, exit_event):
        ...
        while not exit_event.is_set():
            if (time.time()-prctime) > self.write_interval:
                _process_qeue()
                prctime=time.time()
            if self.is_healthy():
...
class Backlog():
    ...
    def refresh(self):
        files = filter(lambda x: os.path.isfile(os.path.join(self.backlog_dir,x)) and re.match("items_[a-zA-Z0-9]+.data$",x),
    ...
    def process(self):
        if self.size() > 0:
            self.log.info("There are %d items in the backlog. Writing items in batches of %d..."%(self.size(),self.writer.batch_size))
            while self.size() > 0:
